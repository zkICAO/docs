...
Commit `613231b` records 340 ACIR opcodes at depth sixteen, measured with `nargo info`, and `nargo info` reproduced that figure during this review. For context, commit `60f7fef` records `predicate_member` at 228 and `sod_ecdsa_p256_sha256_ec512` at 35096. `predicate_member` walks a depth four document path and a depth eight set path, which is the same kind of work, and it carries no signature check, which is where the larger figure goes.
...
The chain mode is more expensive, and the cost is dominated by the signature and by hashing the certificate buffer. How much more is not known here. The measured comparison the repository does contain points the other way from the usual assumption about RSA: `nargo info` during this review gives `sod_rsa2048_v15_sha256_ec512` at 8719 ACIR opcodes against `sod_ecdsa_p256_sha256_ec512` at 35096, so RSA-2048 verification costs roughly a quarter of P-256 verification in these circuits. There is no measurement at 4096 bits and none of a certificate hash, so neither can be extrapolated from that.
...
A third option: fold the anchor into the Passive Authentication circuit instead of composing with it. That removes the salt and the commitment equality check, which is the part of the composition an auditor has to reason about, at the cost of one circuit and one verification key per combination of algorithm, buffer size and registry depth rather than per algorithm and buffer size. The reason the repository gives for splitting circuits, in commit `60f7fef`, is that the signature check dominates and everything downstream costs two or three orders of magnitude less. The measured 340 against 35096 is that shape. It argues for keeping the anchor separate: a registry depth change recompiles only the cheap circuit, and a deployment that runs no anchor simply omits the proof.
