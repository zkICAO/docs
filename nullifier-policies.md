...
Non choosability requires a trust anchor, and the shipped verifier does not require one by default. The Passive Authentication circuit constrains the secret to material that verifies under the Document Signer's public key, and the verifier checks that the nullifier proof carries the same `secret_binding` the Passive Authentication proof published. That is not sufficient on its own. In `bin/sod/ecdsa_p256_sha256_ec512` the public key is a private input, and the circuit publishes only `dsc_commitment(pubkey_hash(pubkey_x, pubkey_y), dsc_salt)` with a prover supplied salt, so the proof says some key signed the document and nothing about whose key it is. `prover/src/verify.rs` says the same in the doc comment on `Verified::signer_registry_root`. A relying party that does not call `Policy::require_anchor` gets `require_trust_anchor: false` from `Policy::new`, and a prover can then generate its own key pair, sign a Security Object it built, choose `(r, s)` and mint as many secrets as it likes. Non choosability therefore holds only against a verifier that requires a trust anchor proof against a registry it published, in either mode: `bin/anchor/dsc_inclusion`, which proves the signer key is in a published set and, as `lib/trust/anchor` states, assumes whoever built the set checked the certificates behind it, or `bin/anchor/csca_chain_rsa2048_sha256_tbs512`, which verifies the Document Signer certificate in circuit and moves the trusted list to the country level.

Non choosability also depends on the signature being canonical. ECDSA verification accepts `(r, s)` exactly when it accepts `(r, n - s)`, so two representations of the same signing act exist and they hash to two different secrets; if both verified, a prover could choose between two secrets for one document and produce two nullifiers in one domain. The pinned backend rejects any `s` above `n/2` and aborts rather than returning false, which forces the low `s` representative and makes the secret canonical, and `p256_rejects_the_high_s_form_of_a_valid_signature` in `lib/core/sig` tests exactly that against the pinned dependency, so the property is checked here rather than assumed of a module comment. RSA raises no such question, since an RSA signature is unique per message and key.

`lib/core/sig` also states that normalization of `s` before witness building belongs to witness preparation: `circuits/fixtures/generator/src/ec.rs` defines `normalize_s`, applies it to every signature it generates in `sign_sha256`, and tests it, and anything that builds witnesses from a real document carries the same rule. The `prover` crate is verification only and builds no witnesses.
...
The nullifier is the cheapest non recursive circuit in the repository, and the document authentication it depends on is the most expensive, by roughly three orders of magnitude. Opcode counts are recorded in `architecture.md`.
