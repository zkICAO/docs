...
Cost of the other zero values, for completeness. `nullifier` asserts `secret != 0` (section 7.14). `context == 0` is rejected everywhere. `domain == 0` is rejected only in `bin/anchor/dsc_inclusion`; see section 11, gap 1.
...
`layout.manifest` is generated from the compiled ABIs and committed in the prover crate. Barretenberg lays out the public parameters in declaration order followed by the return values. Eight tests at the end of `layout.rs` cover this table, four of them by reading the manifest, and they fail if the indices this crate uses drift from it. `cargo test` in `/Users/wstran/Desktop/zkICAO/prover` reports twelve unit tests and one doc test passing on the current tree.
...
1. `policy.context` is not zero, else `ContextNotSet`.
2. One pass over every proof, in order. Its `verification_key_hash` must be in `policy.accepted_keys[circuit]`, else `UntrustedVerificationKey`. This is the downgrade check: without it a proof from a weaker circuit variant would pass on the strength of being a valid proof of something. Then `bb verify` must succeed over the verification key, the proof bytes and the serialised public inputs, else `ProofRejected`; verification is delegated to Barretenberg rather than reimplemented in Rust. Then `public_inputs[domain_index]` must equal `policy.domain`, else `WrongDomain`, since a bundle whose proofs carry different domains is several documents wearing one identity. Then `public_inputs[context_index]` must equal `policy.context`, else `WrongContext`. Security Object proofs are collected as they are seen.
3. Exactly one proof has `Circuit::Sod`. Zero gives `NoSecurityObjectProof`, two or more give `MoreThanOneSecurityObjectProof`.
4. For every `DgExtract` proof, `dg_extract_econtent_binding()` (index 1) equals the Security Object proof's `sod_econtent_binding()` (index 2), else `UnlinkedDataGroup`. The `dg_extract_dg_binding()` values (index 4) are collected.
5. For every `Attributes` proof, `attributes_dg_binding()` (index 0) is one of the bindings collected in step 4, else `UnlinkedDataGroup`. The `attributes_commitment()` values (index 4) are collected.
6. One pass over every proof again. For a `Compare`, `Member` or `Reveal` proof, `referenced_commitment()` (index 1) is one of the commitments collected in step 5, else `UnlinkedCommitment`; a `Reveal` proof also contributes its `field_id` (index 0) and its four revealed elements (indices 2 through 5) to the result. For a `Nullifier` proof, `referenced_commitment()` (index 0) is one of those same commitments, else `UnlinkedCommitment`, and `nullifier_secret_binding()` (index 1) equals the Security Object proof's `sod_secret_binding()` (index 4), else `NullifierFromAnotherDocument`.
7. For every `Anchor` proof, `anchor_dsc_commitment()` (index 3) equals the Security Object proof's `sod_dsc_commitment()` (index 3), else `AnchorForAnotherSigner`. If `policy.registry_root` is set, `anchor_registry_root()` (index 0) must equal it, else `AnchorAgainstAnotherRegistry`. The root seen last is recorded.
8. If `policy.require_trust_anchor` is set and no anchor proof was seen, `NoTrustAnchorProof`.

On success it returns `Verified { nullifier, dsc_commitment, disclosed_fields, signer_registry_root }`: the nullifier value from index 4 of a `Nullifier` proof, the DSC commitment from index 3 of the Security Object proof, the `field_id` and four revealed elements of each `Reveal` proof, and the registry root of an anchor proof when the bundle carried one.
...
`dg_number` on a `DgExtract` proof is never compared to anything. A verifier that needs DG1 specifically must read index 0 itself.

`current_yyyymmdd` on an `Attributes` proof is never compared to anything. It decides how a two digit year resolves, and a prover free to choose it could move a birth date by a century, so a verifier must check it is the date it expected.

`minimum` and `maximum` on a `Compare` proof, and `set_root` on a `Member` proof, are never compared to anything. The statement proved is only as useful as the bounds or the set the verifier required.

`revealed_length` on a `Reveal` proof is checked inside the circuit against the opening, but it is not carried in `Verified.disclosed_fields`; see gap 4.

Signer trust is checked only when the policy asks for it. With `require_trust_anchor` off and no anchor proof in the bundle, `dsc_commitment` is returned and compared against nothing; see gap 2.

## 11. Known gaps

1. `domain` is not constrained to be non-zero anywhere except `bin/anchor/dsc_inclusion`, and `verify_bundle` checks only that `context` is non-zero. A caller that leaves `domain` at its default collapses the commitment and nullifier spaces of every such application into one. Treat a non-zero `domain` as a caller obligation.
2. Signer trust is enforced only when the caller asks for it. `Policy::new` leaves `require_trust_anchor` false and `registry_root` unset, and in that state a bundle proves that some key signed the document and nothing about whose key it is: `verify_bundle` returns `Verified.dsc_commitment` and compares it against nothing unless an anchor proof is present. `Policy` fields are public, so a caller that sets `require_trust_anchor` without `registry_root` gets an anchor proof accepted against any registry, with the root returned but not compared. Use `Policy::require_anchor(root)`, which sets both. Separately, `lib/anchor` notes that verifying the country signing certificate in circuit is not implemented; the registry mode assumes whoever built the set checked the certificates.
...
4. `Verified.disclosed_fields` carries `field_id` and the four packed elements but not `revealed_length`, which sits at index 6 of a `Reveal` proof and is checked inside the circuit. A consumer decoding the packed elements back to bytes has no length in the returned struct and must read index 6 through `PublicInputs::at`.
...
6. `verify_bundle` accepts more than one `Nullifier` proof and reports the last one it iterates, without flagging the duplication. The same holds for `Anchor` proofs and `signer_registry_root`. Each such proof is individually linked to the same commitment, secret binding or DSC commitment, so this is a reporting gap rather than a linkage gap, but a caller that expects at most one value should count them.
